    ...
    <div class="widget-center-container">
      ...
      <div ref="widgetCenterSection" class="widget-center-section">
        <div class="widget-center-content" :style="{ width: contentStyle.width + 'px', height: contentStyle.height + 'px' }">
          <div class="widget-center-wrapper" :style="{ width: widget.width + 'px', height: widget.height + 'px', transform: `scale(${widget.scale / 100})`, background: 'url('+ publicPath + widget.backgroundImage +') 0% 0% / 100% 100%' }" @dblclick="handleLayoutClick">
            <vdr
            ...
            >
              <screen-border :border-box="`${getChartProperty(item.i) ? getChartProperty(item.i).border : 'BorderBox0'}`" :border-title="getChartItem(item.i).chartName" :border-style="{ height: `${item.h}`, width: `${item.w}` }">
                <div v-loading="getChartItem(item.i).loading" :style="{backgroundColor: `${getChartProperty(item.i) ? getChartProperty(item.i).backgroundColor : 'rgba(255, 255, 255, 0.1)'}`}" @dblclick.stop="handleItemClick(item.i)">
...
export default {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      this.hideUpload = fileList.length >= 1
      this.$set(this.dataScreen, 'screenThumbnail', '')
    },
    handleLayoutClick() {
      this.activeTabName = 'first'
    },
