  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { prop: 'beanName', label: 'Spring Bean名称', show: true },
  ...
  methods: {
    ...
    handlePause(row) {
      this.$confirm('是否暂停该任务？', '提示', {
        ...
        pauseJob(row.id || this.ids[0]).then(response => {
          if (response.success) {
    ...
    handleResume(row) {
      this.$confirm('是否恢复该任务？', '提示', {
        ...
        resumeJob(row.id || this.ids[0]).then(response => {
          if (response.success) {
    ...
    handleRun(row) {
      this.$confirm('是否立即执行该任务？', '提示', {
        ...
        runJob(row.id || this.ids[0]).then(response => {
          if (response.success) {
